...
// ___________________________________________________________________Day2______________________________________________________________________________________
...
/*Задача. В переменной min лежит число от 0 до 59. Определите в какую четверть часа попадает это число (в первую, вторую, третью или четвертую).*/
    // let minNum = prompt("enter number from 0 to 59");
    // minNum >= 0 && minNum <= 14 ? alert('first') : 
    // minNum >= 15 && minNum <= 29 ? alert('Second') : 
    // minNum >= 30 && minNum <= 44 ? alert('Third') : 
    // minNum >= 45 && minNum <= 59 ? alert('four') : null;
...
// let message = `Доброе утро, cегодня ${day}\\${month}\\${year}, за бортом отличная погода`;
...
// let guest = prompt('enter password');
// guest === null ? alert('waiting for the enter password') : guest === correctPassword ? alert('Acces allouwed') : alert('Активирована система защиты!')
...
// ordered === total ? alert("Вы забираете весь товар cо склада!") : total > ordered ? alert("Заказ оформлен, с вами свяжется менеджер") : alert("На складе недостаточно твоаров!");
...
// let askNum = Number(prompt("Введите произвольное целое число"));
// askNum === 0 ? alert('Ну и ладно, пока!') : parseInt(askNum) ?  alert('Спасибо!') : alert('Необходимо было ввести целое число!');
...
// let customer = Number(prompt('enter number from 1 to 5'));
...
// const country = prompt('enter coutry');
// let price;
// switch (country) {
//     case "Китай" : 
//     price = "100 кредитов";
...
// const guest = Number(prompt("Введите число мест"));
...
// let a = prompt('enter 1 like a string');

// a === '1' ? (Number(prompt('')) ? alert('no'): prompt('Enter 1')) : alert('No');
...
// a >= 0 && a <= 5 ? alert('true') : alert('false');
